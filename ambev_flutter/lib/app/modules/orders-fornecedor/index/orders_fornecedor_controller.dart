...
class OrdersFornecedorController extends GetxController {
  ...
  getOrders() async {
    loading.value = true;
    orders = await ordersRepository.getFornecedor();
    loading.value = false;
  }

  updateOrder(OrderModel order) async {
    var result = await Get.toNamed('order-details-fornecedor', arguments: {
      'order': order,
    });
